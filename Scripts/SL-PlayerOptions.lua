...
-- or if the higher BPM is within player's acceptable (but not comfortable) range (use median BPM)
local GetUsefulBPM = function(player, StepsOrTrail, MusicRate)
    ...
    return medianBPM, constantpeakBPM, GetDisplayBPMs(player, StepsOrTrail, MusicRate)
end

-- mostly original code with AC and AP added
...
local GetModsAndPlayerOptions = function(player)
	...
	local modslevel = topscreenname == "ScreenEditOptions" and "ModsLevel_Stage" or "ModsLevel_Preferred" -- ScreenPlayerOptions is ScreenEditOptions
	...
	SpeedMod = {
		...
		SaveSelections = function(self, list, pn)
			...
			if type == "AC" then
				...
				if (displaybpms and displaybpms[1] and displaybpms[2]) then
					speed = speed * (displaybpms[2]/medianBPM)
				end
			end
			if type == "CP" then
				type = "M"
