...
local DDR_labels = { 'MARVELOUS', 'PERFECT', 'GREAT', 'GOOD', 'O.K.', 'Miss' }
local DDR_label_colors = { '#ffffbe', '#ffff00', '#00ff05', '#0048ff', '#ff8400', '#ff0018'}


for i=1, #DDR_labels do
	t[#t+1] = LoadFont("DDR A/_DDR A Eval 26px")..{
		Text=DDR_labels[i],
		InitCommand=function(self) self:zoom(0.833):horizalign(right):maxwidth(100) end,
		BeginCommand=function(self)
			self:x( (controller == PLAYER_1 and 45) or -11 )
			if maxCount > 9999 then
				length = math.floor(math.log10(maxCount)+1)
				modifier = controller == PLAYER_1 and -11*(length-4) or 11*(length-4)
				finalPos = 45 + modifier
				finalZoom = 0.833 - 0.1*(length-4)
				self:x( (controller == PLAYER_1 and finalPos) or -finalPos ):zoom(finalZoom)
			end
			self:y((i-1)*28 -16)
			self:diffuse(color(DDR_label_colors[i]))
		end
	}
end
