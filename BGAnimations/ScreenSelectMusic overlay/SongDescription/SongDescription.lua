...
-- GetUsefulBPM() will use timing_data:GetBPMsAndTimes to find median BPM (~ Averarge Constant BPM) and Constant Peak BPM
-- it will help player choose a better scroll speed for some songs where the BPM spikes only for a short burst (use Constant Peak BPM)
-- (some charts like the 18 of ACE FOR ACES from DDR A will have an issue tho, there's an 800bpm section where it's just a hold, need to fix this later)
-- or if the higher BPM is within player's acceptable (but not confortable) range (use median BPM)
local GetUsefulBPM = function(player, StepsOrTrail, MusicRate)
	...
	for i = 1, #segments do
		...
		-- I should use duration in secs instead of beats but whatever, it's harder to catch higher bpm anyway
		local duration = endBeat - startBeat
		if duration > 5 and bpm > constantpeakBPM then -- 5 beats is 750ms at 400bpm or 1.5s at 200bpm or 3s at 100bpm
			constantpeakBPM = bpm
		end
		if duration > 0 then
    ...
    return medianBPM, constantpeakBPM
end
	...
	LoadFont(ThemePrefs.Get("ThemeFont") .. " Normal")..{
		...
		SetCommand=function(self)
			...
			if #GAMESTATE:GetHumanPlayers() == 1 then
				...
				local medianBPM, constantpeakBPM = GetUsefulBPM()
				if tonumber(bpm) == medianBPM and tonumber(bpm) == constantpeakBPM then
					self:settext(bpm or ""):zoom(1)
				else
					if medianBPM == constantpeakBPM then
						self:settext(bpm.." / "..medianBPM or ""):zoom(1)
					else
						self:settext(bpm.." ~"..medianBPM.." ^"..constantpeakBPM or ""):zoom(1)
					end
				end
				return
			end
			...
			local p1mbpm, p1pbpm = GetUsefulBPM(PLAYER_1)
			local p2mbpm, p2pbpm = GetUsefulBPM(PLAYER_2)
			-- if tonumber() recieves something like "%s - %s" from StringifyDisplayBPMs(), it will return nil and "not nil" is true
			if (p1mbpm and not tonumber(p1bpm)) or (not tonumber(p1bpm) == p1mbpm) or (not tonumber(p1bpm) == p1pbpm) then
				if p1mbpm == p1pbpm then
					p1bpm = p1bpm.." / "..p1mbpm
				else
					p1bpm = p1bpm.." ~"..p1mbpm.." ^"..p1pbpm
				end
			end
			if (p2mbpm and not tonumber(p2bpm)) or (not tonumber(p2bpm) == p2mbpm) or (not tonumber(p2bpm) == p2pbpm) then
				if p2mbpm == p2pbpm then
					p2bpm = p2bpm.." / "..p2mbpm
				else
					p2bpm = p2bpm.." ~"..p2mbpm.." ^"..p2pbpm
				end
			end
